...
app.post("/jokes", (req, res) => {
  ...
  if (!jokeText || !jokeType) {
    return res.status(400).json({ error: "Bad Request: Missing jokeText or jokeType" });
  }
...
app.put("/jokes/:id", (req, res) => {
  const jokeId = parseInt(req.params.id);
  const replacementJoke = req.body;
  const { jokeText, jokeType } = replacementJoke;
...
app.delete("/all", (req, res) => {
  const userMasterKey = req.body.masterKey;
  if (userMasterKey !== masterKey) {
